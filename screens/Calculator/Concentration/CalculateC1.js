...
const CalculateC1 = ({ route }) => {
  const { name } = route.params;
  const [concentration2, setConcentration2] = useState('');
  const [volume2, setVolume2] = useState('');
  const [concentration1, setConcentration1] = useState(null);
  const [volume1, setVolume1] = useState(null);

  const calculateConcentration1 = () => {
    if (concentration2 && volume2) {
      const c2 = parseFloat(concentration2);
      const v2 = parseFloat(volume2);

      const c1 = (c2 * v2) / volume1;
      setConcentration1(c1.toFixed(2)); // rounding to 2 decimal places
    }
  };
...
<Text style={styles.label}>Volume 1 (V1):</Text>
          ...
          <View style={styles.buttonContainer}>
            ...
            <UIButton title='Calculate C1' onPress={calculateConcentration1} />
          </View>

          {concentration1 !== null ? (
            <Text style={styles.resultLabel}>Concentration 1 (C1) = {concentration1}</Text>
          ) : null}
        </View>
      </ScrollView>
    </SafeAreaView>
